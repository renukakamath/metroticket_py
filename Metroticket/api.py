...
# compiled_contract_path = 'C:/Users/renuk/Downloads/Finale_backup/Metroticket_recent/Metroticket/node_modules/.bin/build/contracts/metro.json'
# # compiled_contract_path = 'F:/NGO/node_modules/.bin/build/contracts/medicines.json'
# # Deployed contract address (see `migrate` command output: `contract address`)
# deployed_contract_address = '0x68Ef7f4b4FE9E1228af45C0610Aa68245bDF5587'
...
def AndroidBarcodeQrExample4():
	...
	if res3:
		...
		if res1:
			...
			if res2[0]['user_type']=="staffsmetro":
				...
				if amts1==0.0:
					...
					# message = contract.functions.add_transactions(id,int(logid),str(sums),latitude,longitude,d,'out','complete',total).transact()
		...
		if res4[0]['user_type']=="staffsmetro":
			...
			# 	contract = web3.eth.contract(address=deployed_contract_address, abi=contract_abi)
			# 	id=web3.eth.get_block_number()
			# message = contract.functions.add_transactions(id,int(logid),'0',fplace,'metro',d,'In','Incomplete',total).transact()
			...
			# 	contract = web3.eth.contract(address=deployed_contract_address, abi=contract_abi)
			# 	id=web3.eth.get_block_number()
			# message = contract.functions.add_transactions(id,int(logid),'0',fplace,'auto',d,'In','Incomplete',total).transact()
			...
			# 	contract = web3.eth.contract(address=deployed_contract_address, abi=contract_abi)
			# 	id=web3.eth.get_block_number()
			# message = contract.functions.add_transactions(id,int(logid),'0',fplace,'boat',d,'In','Incomplete',total).transact()
			...
			# 	contract = web3.eth.contract(address=deployed_contract_address, abi=contract_abi)
			# 	id=web3.eth.get_block_number()
			# message = contract.functions.add_transactions(id,int(logid),'0',fplace,'bus',d,'In','Incomplete',total).transact()
			...
			# 	contract = web3.eth.contract(address=deployed_contract_address, abi=contract_abi)
			# 	id=web3.eth.get_block_number()
			# message = contract.functions.add_transactions(id,int(logid),'0',fplace,'car',d,'In','Incomplete',total).transact()
...
def searchplace():
	...
	q="SELECT * FROM `requestdetails`  where fplaces like '%s' and tplaces like '%s'"%(search,se) 
	res=select(q)
...
@api.route('/viewsearchplace')
def viewsearchplace():
	data={}
	# lid=request.args['lid']
	

	q="SELECT * FROM `requestdetails`  where rates!='progress'  " 
	res=select(q)
...
def AddRequestss():
	...
	if flatis=="" or flongis=="" or tlatis=="" or tlongis=="":
		...
		# amounts = rounded_cost

		# Convert amounts to a string
		...
		q="insert into requestdetails values(null,'%s','%s','%s','%s','%s','%s','%s')"%(rid,fplace,tplace,output_str,types,amounts[0],amounts[0])
		id=insert(q)
